...
def run_train_gcn_for_link_prediction(model, train_y, optimizer,
                  data_with_features, pos_edges_ind, neg_edges_ind):
    """
    ...
    def train():
        ...
        # HERE check out does output of model returns? (the same order or not?)
        # logit = model(data_with_features, edges, edges_weight)
        logits, accs = torch.sigmoid(model(data_with_features,pos_edges_ind, neg_edges_ind)), []

        # loss = F.binary_cross_entropy_with_logits(logit, link_labels)
        ...
        loss = F.binary_cross_entropy_with_logits(logits.float(), train_y.float())
        optimizer.step()
        return model
...
def run_test_gcn_for_link_prediction(model, y , optimizer, data_with_features,
                 train_edges_ind, test_edges_ind, edges_weight):
    
    def performance_per_epoch():
        train_test_pred = []
        train_test_accs = []
        train_test_pred_prob = []
        for edges_ind, label in zip([train_edges_ind, test_edges_ind], y):
            logits, accs = torch.sigmoid(
                model(data_with_features, edges_ind[0], edges_ind[1])), []
            pred_proba = logits
            # pred = pred_proba.max(1)[1]
            pred = (pred_proba > 0.5).int()
            # acc = pred.eq(label[0]).sum().item() / label[0].shape[0]
    ...
    def test():
        model.eval()
        return performance_per_epoch()

    return test()


def run_gcn_for_link_prediction(data, data_with_features, split, task, splitted_edges_dir, split_by_node):
    ...
    def get_train_test_set():
        ...
        data_with_features.index = data_with_features.index.map(str)
        tmp = data_with_features.reindex(x_train_ind[:,0]).dropna().to_numpy()
        tmp_1 = data_with_features.reindex(x_train_ind[:,1]).dropna().to_numpy()
        x_train = np.concatenate([tmp, tmp_1], axis=1)

        tmp = data_with_features.reindex(x_test_ind[:,0]).dropna().to_numpy()
        tmp_1 = data_with_features.reindex(x_test_ind[:,1]).dropna().to_numpy()
    ...
    def prepare_argument_data(data_with_features, y_train, y_test):
        ...
        num_label = np.unique(y_train).shape[0]
        # edges = np.array(list(data.G.edges)).T
        train_edges = x_train_ind.T
        test_edges = x_test_ind.T
        ...
        device = torch.device('cuda' if torch.cuda.is_available() else 'cpu')
        model = Net_for_link_prediction(num_feature, num_label).to(device)
        optimizer = torch.optim.Adam([
            dict(params=model.reg_params, weight_decay=5e-4),
            dict(params=model.non_reg_params, weight_decay=0)
        ], lr=0.01)

        def convert_node_name_to_its_index(nodes_with_features, edges):
            # convert nodes to its index
            node_num = np.unique(list(nodes_with_features.index)).shape[0]
            assert node_num == nodes_with_features.shape[0], ' '
        ...
        train_edges_ind = convert_node_name_to_its_index(data_with_features, train_edges)
        test_edges_ind = convert_node_name_to_its_index(data_with_features, test_edges)

        # convert to tensor
        data_with_features = torch.tensor(data_with_features.to_numpy()).float()
        # edges_ind = torch.tensor(edges_ind).long()
        train_edges_ind = torch.tensor(train_edges_ind).long()
        test_edges_ind = torch.tensor(test_edges_ind).long()
        edges_weight = torch.tensor(edges_weight)
        y_train = torch.tensor(y_train).long()
        y_test = torch.tensor(y_test).long()

        return data_with_features, y_train,y_test, model, optimizer, train_edges_ind, test_edges_ind, edges_weight

    data_with_features_ts, y_train_ts, y_test_ts, model, optimizer, train_edges_ind_ts, test_edges_ind_ts , edges_weight_ts = prepare_argument_data(
        data_with_features, y_train, y_test)

    train_acc_hist = []
    test_acc_hist = []

    pos_train_edges_ind_ts  = train_edges_ind_ts[:, np.where(y_train == 1)[0]]
    neg_train_edges_ind_ts  = train_edges_ind_ts[:, np.where(y_train == 0)[0]]
    pos_test_edges_ind_ts  = test_edges_ind_ts[:, np.where(y_test == 1)[0]]
    neg_test_edges_ind_ts  = test_edges_ind_ts[:, np.where(y_test == 0)[0]]
    pos_train_edges_weight_ind_ts = train_edges_ind_ts[:, np.where(y_train == 1)[0]]
    neg_train_edges_weight_ind_ts = train_edges_ind_ts[:, np.where(y_train == 0)[0]]



    for i in range(1, 50):
        model = run_train_gcn_for_link_prediction(model,
                              y_train_ts, optimizer,
                              data_with_features_ts,
                              pos_train_edges_ind_ts,
                                neg_train_edges_ind_ts,)



        (train_acc, tmp_test_acc), (y_train_pred, y_test_pred), (
            y_train_pred_proba, y_test_pred_proba) = run_test_gcn_for_link_prediction(model,
                                                                  [y_train_ts, y_test_ts],
                                                                  optimizer,
                                                                  data_with_features_ts,
                                                                  [pos_train_edges_ind_ts,
                                                                  neg_train_edges_ind_ts],
                                                                   [pos_test_edges_ind_ts, 
                                                                    neg_test_edges_ind_ts],
                                                                  edges_weight_ts)
        test_acc = tmp_test_acc
        log = 'Epoch: {:03d}, Train: {:.4f}, Test: {:.4f}'
        # print(log.format(i, train_acc, test_acc))
    ...
    train_report_np, train_columns, train_index = report_performance(y_train_np,
                                                                     y_train_pred_np,
                                                                     np.vstack((y_train_pred_proba_np, 1-y_train_pred_proba_np)).T,
                                                                     # y_train_pred_proba_np,
                                                                     np.unique(
                                                                         y_train_np),
                                                                     plot=True,
    ...
    test_report_np, test_columns, test_index = report_performance(y_test,
                                                                  y_test_pred_np,
                                                                  np.vstack((y_test_pred_proba_np,1-y_test_pred_proba_np)).T,
                                                                  np.unique(
                                                                      y_test),
                                                                  plot=True,
                                                                  verbose=True,
...
def run_gcn_with_specified_classifier_for_link_prediction(data=None, x_with_features=None,
    ...
    run_gcn_for_link_prediction(data, x_with_features, split, task,
                                splitted_edges_dir=splitted_edges_dir,
                                split_by_node=split_by_node)
...
class Net_for_link_prediction(torch.nn.Module):
    def __init__(self, num_feature, num_classes):
        super(Net_for_link_prediction, self).__init__()
        # hidden2 = 32
        hidden3 = 16
        self.conv1 = GCNConv(num_feature, hidden3)
        # self.conv2 = GCNConv(hidden2, hidden3, cached=True)
        self.conv3 = GCNConv(hidden3, num_classes)
    ...
    def forward(self, x, pos_edge_index, neg_edge_index):
        # edge_weight = edge_weight.type(torch.double)
        x = F.relu(self.conv1(x.float(), pos_edge_index.long()))
        x = F.dropout(x, training=self.training)
        # x = self.conv2(x.float(), edge_index.long(), edge_weight)
        # x = F.relu(x)
        x = F.dropout(x, training=self.training)
        x = self.conv3(x.float(), pos_edge_index.long())
