...
class GeneDiseaseGeometricDataset(InMemoryDataset, Dataset):
    ...
    def run_split_train_test(self, x_np, y_np, split, stratify):
        """
    ...
    def create_stratified_k_fold(self, data, x_np, y_np, k_fold,
        ...
        if stratify:
            if not split_by_node:
                for train_set_ind, test_set_ind in self.run_cross_validation(
                        x_np,
                        y_np, k_fold,
                        stratify,
                ):
                    ...
                    splitted_edges_dict = {'edges_train_set': train_set, 'edges_test_set': test_set}

                    pickle.dump( splitted_edges_dict, open( splitted_edges_file_path, "wb" ) )

                    self.stratified_k_fold.append({'train_set':train_set, 'test_set':test_set})

                    ## figure out how to save to bin using pickel
                    ## use case of splitted_nodes is as folloed
                    ##         splitted_nodes[1] = {'train_set': list of nodes in train_set, 'test_set': list_of_nodes in test_set}
            else:
                ...
                for ind, (train_set_ind, test_set_ind) in enumerate(self.run_cross_validation(
                    ...
                    train_pos_edges = np.array([[i, j, '1.0'] for i in
                                                ...
                                                self.original_GeneDisease_edges.neighbors(
                                                    i)])
                    selected_non_existed_edges = selected_edges_with_label_dict['non_existed_edges']
                    ...
                    selected_train_neg_edges = selected_non_existed_edges.iloc[selected_train_neg_edges_ind]

                    selected_non_existed_edges = selected_non_existed_edges.drop(
                        selected_non_existed_edges.index[selected_train_neg_edges_ind])
                    ...
                    selected_test_neg_edges_ind = np.random.choice(
                        selected_non_existed_edges.shape[0], len(test_pos_edges),
                        replace=False)
                    # selected_test_neg_edges = \
                    # selected_edges_with_label_dict['non_existed_edges'].iloc[
                    #     selected_test_neg_edges_ind]
                    selected_test_neg_edges = selected_non_existed_edges.iloc[selected_test_neg_edges_ind]
                ...
                splitted_edges_dir = '\\'.join(splitted_edges_file_path.split('\\')[:-1])
    ...
    # def run_split_cross_validation_for_link_prediction(self, split, stratify):
    def run_split_cross_validation_for_link_prediction(self, data, k_fold,
        ...
        if path.exists(edges_with_pos_neg_as_label):
            ...
            # TODO here>>-2 what are the expected output from self.run_cross_valdiation => what are the behavior that I want?
            ## Ans I want to yield list(((train_set_1, test_set_1), (train_set_2, test_set_2)
            # TODO here>>-3
            ## > create selected_pos_neg_edges_with_label
            ##     >> where do I set it?
            ##     >> where do I validate it? => valiate that the value is set (not None)
            ##          >> make sure that its value cannot be reset. (pass in variable to make it explicit)
            ## > pass selected_pos_neg_edges_with_label into cross validation and it will return ind.
            ## > when running for loop on data.split_cross_validation(), it is expected to return list of train_set_ind, test_set_ind
            ## > train_set_ind and test_set_ind are then used to selected from selected_pos_neg_edges_with_label

            # spliiting pos
            # train_set_ind, test_set_ind = self.run_cross_validation(
            #     pos_edges_with_label_np[:,:-1], pos_edges_with_label_np[:,-1], split,
            #     stratify)
            ...
            x_np, y_np = selected_shuffled_pos_neg_edges_with_label[:,
                         :-1], selected_shuffled_pos_neg_edges_with_label[:,
                               -1]

            # edges_with_features = []
            # edges_label = []
            # for node1, node2, _ in x_np.tolist():
            #     edges_instance_with_features = np.concatenate(
            #         [selected_shuffled_pos_neg_edges_with_label.loc[node1],
            #          selected_shuffled_pos_neg_edges_with_label.loc[node2]]).tolist()
            ...
            return self.create_stratified_k_fold(data, x_np, y_np, k_fold,
    ...
    def run_split_train_test_for_node_classification(self, split, stratify):
        ...
        return self.run_split_train_test(self.diseases_np, disease_class,
                                         split,
                                         stratify)
    ...
    def split_cross_validation(self, data, k_fold, stratify=None, task=None,
        ...
        if len(self.stratified_k_fold) == 0 :
            if path.exists(splitted_edges_dir) and path.exists(
                ...
                splitted_edges_dict = pickle.load( open( splitted_edges_file, "rb" ) )

                for key, edges_train_test_set_dict in splitted_edges_dict.items():
                    edges_train_set = edges_train_test_set_dict['edges_train_set']
                    edges_test_set = edges_train_test_set_dict['edges_test_set']

                    if key not in self.stratified_k_fold:
                        # self.stratified_k_fold[key] = {'train_set': edges_train_set, 'test_set': edges_test_set}
                        self.stratified_k_fold.append({'train_set':edges_train_set, 'test_set':edges_test_set})
                    else:
                        raise ValueError(f'key is already existed in self.stratified_k_fol  => There are dubplicate keys in {splitted_edges_file}')
                print()
    ...
    def split_train_test(self, split, stratify=None, task=None,
        ...
        if reset_train_test_split:
            if task == 'node_classification':
                return self.run_split_train_test_for_node_classification(split,
                                                                         stratify)
            ...
            return self.train_set, self.test_self
